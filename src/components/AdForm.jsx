...
const totalImages = files.length + (initialData.imageUrls?.length || 0);
if (totalImages > 3) {
...
const handleSubmit = async (e) => {
  e.preventDefault();
  if (!user) {
    toast.error('Please log in to create or edit an ad');
    return;
  }

  setLoading(true); // This should come right after the user check
...
return (
<form onSubmit={handleSubmit} className="space-y-5 bg-gray-900 bg-opacity-80 text-white p-6 rounded-2xl shadow-md backdrop-blur-md border border-indigo-500 mt-6" >
